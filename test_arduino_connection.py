...
def test_arduino_connection(port, cycle_count=3, delay=1.0):
    """
    아두이노 연결을 테스트하고 LED 색상을 순환합니다.
    ...
    try:
        ...
        print(f"LED 색상 테스트 시작 (순환 횟수: {cycle_count})...")
...
def main():
    parser = argparse.ArgumentParser(description='아두이노 연결 및 LED 제어 테스트')
    ...
    # 아두이노 연결 테스트
    test_arduino_connection(args.port, args.cycles, args.delay)
